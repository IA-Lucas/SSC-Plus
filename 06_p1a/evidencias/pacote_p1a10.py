#!/usr/bin/env python3
"""Gerador deterministico de pacote de revisao — SSC+ P1-A.10.

DERIVADO MECANICAMENTE de `pacote_p1a37.py`, copia VERBATIM, com UMA
divergencia declarada e nada alem dela:

  `_classificar` passa a tratar os status `R` (rename) e `C` (copy) do
  `git diff --name-status -z`, que vem com TRES campos e nao dois. O
  gerador da P1-A.3.7 consumia dois campos sempre; na primeira rename do
  acervo (as quatro decisoes fundadoras movidas para
  `00_governanca-experimental/` em 2026-08-12) ele leu o caminho antigo
  como modificado e PAROU no cat-file. Rename entra como REMOVIDO (hash
  do blob em BASE, caminho antigo) + disposicao normal do caminho novo;
  copy entra so pelo caminho novo, porque o antigo continua em ALVO.

Por que copia e nao edicao: `pacote_p1a37.py` e o instrumento congelado
que gerou os pacotes ja julgados, e a suite o exerce com o SHA dele
embutido no proprio pacote. Editar o congelado mudaria retroativamente
o objeto que os revisores viram.

O texto original da P1-A.3.7 segue abaixo, intacto.
"""
_DOCSTRING_ORIGINAL_P1A37 = """Gerador deterministico de pacote de revisao — SSC+ P1-A.3.7.
...
def _classificar(base: str, alvo: str) -> dict:
    ...
    A DIVERGENCIA declarada desta copia: `R`/`C` vem com TRES campos no
    `-z` (status, caminho antigo, caminho novo). Rename: o antigo entra
    em removidos (hash em BASE) e o novo segue a disposicao normal.
    Copy: so o novo entra — o antigo continua vivo em ALVO e nao mudou.
    """
    ...
    while i < len(campos) - 1:
        estado = campos[i][:1]
        if estado in ("R", "C"):
            antigo, novo = campos[i + 1], campos[i + 2]
            i += 3
            if estado == "R":
                classes["removidos"].append(antigo)
            classes["novos"].append(novo)
            continue
        rel = campos[i + 1]
        i += 2
...
def main(argv=None) -> int:
    ...
    if len(argv) != 3:
        print("uso: pacote_p1a10.py <BASE> <ALVO> <saida>", file=sys.stderr)
        return 2
